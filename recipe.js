...
// INIT
//////////////////////////////////////////
module.exports = ({ rounds } = { rounds: 30 }) => {
  let recipe = [...Array(rounds)].map((x) => {
  ...
  return {
    recipe,
    pattern: displayPattern(recipe),
    beats: numOfBeats(recipe),
    moves: recipe.length,
    defineMoves: defineMoves(dictionary),
  }
}
...
function defineMoves(dictionary) {
  return dictionary.map((move) => `${move.symbol} = ${move.move}`).join('\n')
}
